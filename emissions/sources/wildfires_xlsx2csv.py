...
for year in range(1986,2021):
    print(year)
    prefixed = [filename for filename in os.listdir('./SVN_2022/') if filename.startswith("SVN_2022_{:4d}".format(year))]
    df = pd.read_excel('./SVN_2022/'+prefixed[0],sheet_name="Table4(V)")
    dfval = df.values
    
    blah[i,:] = [year,dfval[10,4],dfval[10,5],dfval[10,8]]
    i+=1

#%%
import matplotlib.pyplot as plt
plt.plot(blah[:,0],blah[:,3],"o-",color="red")
plt.xlabel("leto")
plt.ylabel("zgorela površina [ha]")
