...
for curr_date in np.arange(start_date, end_date, np.timedelta64(1, 'D')):
    ...
    if day == 1 and month == 1:
        call("mv *.nc ../sources/tcw_{0}.nc".format(year),shell=True)
    else:
        call("mv *.nc ../temp/tcw_append.nc",shell=True)
        
        # Open and concatenate the files along the "time" dimension
        ds = xr.open_mfdataset(["../sources/tcw_{0}.nc".format(year),"../temp/tcw_append.nc"], combine="nested", concat_dim="time")
        
        # Save the concatenated file
        ds.to_netcdf("../temp/new.nc".format(year))
        call("mv ../temp/new.nc ../sources/tcw_{0}.nc".format(year),shell=True)
